...
void sample_grid(struct bic_int *img, int step_x, int step_y, unsigned char sigma)
{   ppm_image *image = img->new_image;
    int p = image->x / step_x; // nr of pixels per line
    int q = image->y / step_y; // nr of pixels per column

    long id = img->id;
    int start = id * (double)p / img->P;
    int end = min((id + 1) * (double)p/ img->P, p);
...
void rescale_image(struct bic_int *img)
{
    uint8_t sample[3];
    long id = img->id;
...
void *parallel(void *arg) {
    ...
    sample_grid(img,STEP,STEP,SIGMA);
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        for (id = 0; id < num_threads; id++)
        {
            ...
            x = pthread_create(&threads[id], NULL, parallel, img);
